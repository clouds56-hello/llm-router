...
fn coerce(raw: &str, prior: Option<&Item>) -> Item {
    ...
    if let Some(Item::Value(v)) = prior {
        match v {
            EditValue::Boolean(_) => {
                if let Ok(b) = raw.parse::<bool>() { return value(b); }
            }
            EditValue::Integer(_) => {
                if let Ok(n) = raw.parse::<i64>() { return value(n); }
            }
            EditValue::Float(_) => {
                if let Ok(n) = raw.parse::<f64>() { return value(n); }
            }
            EditValue::Array(_) => {
...
fn cmd_edit_profiles() -> Result<()> {
    let path = profiles::user_profiles_path()
        .ok_or_else(|| anyhow!("could not resolve user profiles path"))?;
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).ok();
    }
    if !path.exists() {
        std::fs::write(&path, b"# User-defined personas. See built-in profiles.toml for schema.\n")?;
    }
    open_in_editor(&path)?;
    let raw = std::fs::read_to_string(&path)?;
...
fn cmd_path(path: &std::path::Path, want_profiles: bool) -> Result<()> {
    if want_profiles {
        let p = profiles::user_profiles_path()
            .ok_or_else(|| anyhow!("could not resolve user profiles path"))?;
        println!("{}", p.display());
    } else {
...
fn lookup<'a>(doc: &'a DocumentMut, segments: &[String]) -> Option<&'a Item> {
    ...
    if segments.len() >= 2 && segments[0] == "accounts" {
        ...
        let entry = arr.iter().find(|t| {
            t.get("id").and_then(|i| i.as_str()) == Some(segments[1].as_str())
        })?;
...
fn descend_item<'a>(item: &'a Item, rest: &[String]) -> Option<&'a Item> {
    if rest.is_empty() { return Some(item); }
    match item {
...
fn remove(doc: &mut DocumentMut, segments: &[String]) -> bool {
    ...
    if segments.len() >= 2 && segments[0] == "accounts" {
        ...
        let Some(entry) = arr.iter_mut().find(|t| {
            t.get("id").and_then(|i| i.as_str()) == Some(segments[1].as_str())
        }) else {
            return false;
        };
    ...
    let Some(item) = doc.get_mut(&segments[0]) else { return false; };
...
fn remove_from_table(t: &mut Table, segments: &[String]) -> bool {
    ...
    let Some(item) = t.get_mut(&segments[0]) else { return false; };
    let Some(inner) = item.as_table_mut() else { return false; };
...
mod tests {
    ...
    fn doc(s: &str) -> DocumentMut { s.parse().unwrap() }
    ...
    fn insert_account_field() {
        ...
        insert(&mut d, &["accounts".into(), "work".into(), "behave_as".into()], value("opencode")).unwrap();
        let s = d.to_string();
