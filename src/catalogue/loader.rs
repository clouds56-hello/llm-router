...
pub fn cache_path() -> Option<PathBuf> {
    directories::ProjectDirs::from("", "", "llm-router")
        .map(|d| d.cache_dir().join("catalogue.json"))
}
...
fn parse_embedded() -> Catalogue {
    serde_json::from_slice(EMBEDDED)
        .expect("embedded models.dev snapshot must parse — fix build.rs")
}
...
pub async fn fetch_and_persist(http: &reqwest::Client, url: &str) -> Result<UpdateReport> {
    ...
    let parsed: Catalogue = serde_json::from_slice(&body)
        .with_context(|| format!("parse {url} as models.dev catalogue"))?;
    if parsed.is_empty() {
