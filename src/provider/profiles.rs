...
impl Profiles {
            ...
            for (scope, section) in scopes {
                ...
                for (k, v) in tbl {
                    ...
                    let val = v.as_str().ok_or_else(|| {
                        anyhow!("[{persona}.{scope}].{k} must be a string")
                    })?;
    ...
    pub fn resolve(&self, persona: &str, upstream: &str) -> Option<ResolvedProfile> {
        ...
        let mut out = ResolvedProfile { verified: true, ..Default::default() };
...
mod tests {
    ...
    fn copilot_default_verified() {
        ...
        assert!(r.verified);
        assert_eq!(r.headers.get("user-agent").map(String::as_str),
