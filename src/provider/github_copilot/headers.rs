...
pub fn token_exchange_headers(github_token: &str, h: &CopilotHeaders) -> Result<HeaderMap> {
  ...
  m.insert(AUTHORIZATION, HeaderValue::from_str(&format!("token {github_token}"))?);
...
pub fn copilot_request_headers(
  ...
  let mut m = HeaderMap::new();
  m.insert(AUTHORIZATION, HeaderValue::from_str(&format!("Bearer {api_token}"))?);
  m.insert(
    ACCEPT,
  ...
  for (k, v) in &h.extra_headers {
    let name = HeaderName::from_bytes(k.as_bytes()).with_context(|| format!("invalid header name {k:?}"))?;
    let val = HeaderValue::from_str(v).with_context(|| format!("invalid header value for {k:?}"))?;
    m.insert(name, val);
  }
  Ok(m)
}
...
fn insert_str(m: &mut HeaderMap, name: &'static str, value: &str) -> Result<()> {
  ...
  let v = HeaderValue::from_str(value).with_context(|| format!("invalid header value for {name}"))?;
  m.insert(n, v);
  Ok(())
}
